...
class BarsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bar" do
    assert_difference('Bar.count') do
      post bars_url, params: { bar: { name: @bar.name, organization_id: @bar.organization_id } }
    end
  ...
  test "should update bar" do
    patch bar_url(@bar), params: { bar: { name: @bar.name, organization_id: @bar.organization_id } }
